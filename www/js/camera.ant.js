...
function sendSnapShot(base64){
	var request = new XMLHttpRequest();
	  alert("sendSnapShot");
		request.open('POST', '/php/save_photos.php', true);
		...
		request.onload = function() {
			...
			if (request.status >= 200 && request.status < 400) {
				...
				//Mostrar informações
				alert(data.img);
				document.querySelector("#imagemConvertida").setAttribute("src", "https://www.dogmo.app/php/"+data.img);
				document.querySelector("#caminhoImagem a").setAttribute("href", "https://www.dogmo.app/php/"+data.img);
